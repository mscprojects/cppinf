...
// Applies Qwen attention to rank-3 [batch, sequence, hidden] hidden states while appending K/V tensors to cache.
// sequence_lengths mark the valid suffix length for each batch row, and cache carries the already materialized prefix
// length for every batch row.
