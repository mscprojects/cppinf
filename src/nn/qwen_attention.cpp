...
tensors::Tensor split_heads(const tensors::TensorView& input, std::size_t head_count, std::string_view result_name) {
    ...
    const auto merged_head_size = checked_positive_dim_to_size(dims[1], fmt::format("{} merged head size", result_name));
    if (merged_head_size % head_count != 0) {
        throw std::invalid_argument(fmt::format("{} requires the merged head size to divide by the head count.", result_name));
    }

    const auto head_size = merged_head_size / head_count;
    auto result = tensors::Tensor::zeros(make_result_info(
        ...
        tensors::Shape({static_cast<std::int64_t>(head_count), static_cast<std::int64_t>(sequence_length),
                        static_cast<std::int64_t>(head_size)})));

    const auto head_bytes = head_size * sizeof(float);
    for (std::size_t head_index = 0; head_index < head_count; ++head_index) {
        for (std::size_t sequence_index = 0; sequence_index < sequence_length; ++sequence_index) {
            const auto source_offset = (sequence_index * merged_head_size + head_index * head_size) * sizeof(float);
            const auto destination_offset = ((head_index * sequence_length) + sequence_index) * head_bytes;
            std::memcpy(result.mutable_data().data() + destination_offset, input.data().data() + source_offset, head_bytes);
        }
    }
...
tensors::Tensor merge_heads(const tensors::TensorView& input, std::string_view result_name) {
    ...
    const auto head_size = checked_positive_dim_to_size(dims[2], fmt::format("{} head size", result_name));
    const auto merged_head_size = head_count * head_size;
    ...
    for (std::size_t head_index = 0; head_index < head_count; ++head_index) {
        for (std::size_t sequence_index = 0; sequence_index < sequence_length; ++sequence_index) {
            ...
            const auto destination_offset = (sequence_index * merged_head_size + head_index * head_size) * sizeof(float);
            std::memcpy(result.mutable_data().data() + destination_offset, input.data().data() + source_offset, head_bytes);
        }
    }
...
tensors::Tensor repeat_heads(const tensors::TensorView& input, std::size_t target_head_count, std::string_view result_name) {
    ...
    if (target_head_count % input_head_count != 0) {
        throw std::invalid_argument(fmt::format("{} requires the target head count to divide by the input head count.",
                                                result_name));
    }
    ...
    const auto bytes_per_head = sequence_length * head_size * sizeof(float);
...
void validate_qwen_attention_inputs(const tensors::TensorView& hidden_states, const tensors::TensorView& q_proj_weight,
                                    ...
                                    std::size_t num_key_value_heads, float rope_base) {
    if (num_attention_heads == 0 || num_key_value_heads == 0) {
        throw std::invalid_argument("qwen_attention requires non-zero head counts.");
    }
    ...
    if (hidden_states.tensor_info().dtype != q_proj_weight.tensor_info().dtype ||
        hidden_states.tensor_info().dtype != k_proj_weight.tensor_info().dtype ||
        hidden_states.tensor_info().dtype != v_proj_weight.tensor_info().dtype ||
        hidden_states.tensor_info().dtype != o_proj_weight.tensor_info().dtype) {
    ...
    if (hidden_size % num_attention_heads != 0) {
        throw std::invalid_argument("qwen_attention requires hidden size to divide by num_attention_heads.");
    }

    const auto head_size = hidden_size / num_attention_heads;
    validate_projection_weight(q_proj_weight, "qwen_attention q_proj_weight", hidden_size, hidden_size);
    validate_projection_weight(k_proj_weight, "qwen_attention k_proj_weight", num_key_value_heads * head_size, hidden_size);
    validate_projection_weight(v_proj_weight, "qwen_attention v_proj_weight", num_key_value_heads * head_size, hidden_size);
    validate_projection_weight(o_proj_weight, "qwen_attention o_proj_weight", hidden_size, hidden_size);
    if (rope_base <= 0.0f) {
        throw std::invalid_argument("qwen_attention requires a positive rope base.");
    }
}
...
tensors::Tensor qwen_attention(const tensors::TensorView& hidden_states, const tensors::TensorView& q_proj_weight,
                               const tensors::TensorView& k_proj_weight, const tensors::TensorView& v_proj_weight,
                               const tensors::TensorView& o_proj_weight, std::size_t num_attention_heads,
                               std::size_t num_key_value_heads, std::size_t sequence_position_offset, float rope_base) {
    validate_qwen_attention_inputs(hidden_states, q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight,
                                   num_attention_heads, num_key_value_heads, rope_base);

    std::optional<tensors::Tensor> hidden_storage;
    ...
    const auto hidden_states_f32 = maybe_cast_to_f32(hidden_states, hidden_storage);
    const auto q_proj_weight_f32 = maybe_cast_to_f32(q_proj_weight, q_proj_weight_storage);
    const auto k_proj_weight_f32 = maybe_cast_to_f32(k_proj_weight, k_proj_weight_storage);
    const auto v_proj_weight_f32 = maybe_cast_to_f32(v_proj_weight, v_proj_weight_storage);
    const auto o_proj_weight_f32 = maybe_cast_to_f32(o_proj_weight, o_proj_weight_storage);
    ...
    const auto query_heads = split_heads(query_projection.view(), num_attention_heads, "qwen_attention_query_heads");
    const auto key_heads = split_heads(key_projection.view(), num_key_value_heads, "qwen_attention_key_heads");
    const auto value_heads = split_heads(value_projection.view(), num_key_value_heads, "qwen_attention_value_heads");

    const auto rotated_query = apply_rope(query_heads.view(), sequence_position_offset, rope_base);
    const auto rotated_key = apply_rope(key_heads.view(), sequence_position_offset, rope_base);
    ...
    const auto attention_output = causal_self_attention(rotated_query.view(), repeated_key.view(), repeated_value.view());
