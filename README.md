...
`cppinf` is a very small C++20 LLM inference engine. It is mainly focused on the Qwen3 architecture right now, with CPU
reference inference for Hugging Face checkpoints, safetensors loading, a minimal Hugging Face tokenizer, cached
...
```sh
just run inspect hf ~/Models/Qwen3-0.6B --limit 32
```

Show every tensor in the weights file:

```sh
just run inspect hf ~/Models/Qwen3-0.6B --all
```

Generate text:

```sh
just run run hf ~/Models/Qwen3-0.6B --prompt "Hello" --max-new-tokens 32 --temperature 0
```
