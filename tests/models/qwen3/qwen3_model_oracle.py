...
def qwen_attention_module(hidden_states: torch.Tensor,
...
def qwen_mlp_module(hidden_states: torch.Tensor,
...
def qwen_decoder_block_module(hidden_states: torch.Tensor,
    ...
    mlp_out = qwen_mlp_module(mlp_input, params["gate"].float(), params["up"].float(), params["down"].float())
...
if __name__ == "__main__":
    ...
    embed = round_tensor(torch.empty(vocab_size, hidden_size).uniform_(-1.1, 1.1)).to(torch.bfloat16)
    layer_params = []
    for _ in range(num_hidden_layers):
        layer_params.append(
            {
                "input_ln": round_tensor(torch.empty(hidden_size).uniform_(0.5, 1.5)).to(torch.bfloat16),
                "post_ln": round_tensor(torch.empty(hidden_size).uniform_(0.5, 1.5)).to(torch.bfloat16),
                "q_proj": round_tensor(torch.empty(num_attention_heads * head_dim, hidden_size).uniform_(-1.1, 1.1)).to(torch.bfloat16),
                "q_norm": round_tensor(torch.empty(head_dim).uniform_(0.5, 1.5)).to(torch.bfloat16),
                "k_proj": round_tensor(torch.empty(num_key_value_heads * head_dim, hidden_size).uniform_(-1.1, 1.1)).to(torch.bfloat16),
                "k_norm": round_tensor(torch.empty(head_dim).uniform_(0.5, 1.5)).to(torch.bfloat16),
                "v_proj": round_tensor(torch.empty(num_key_value_heads * head_dim, hidden_size).uniform_(-1.1, 1.1)).to(torch.bfloat16),
                "o_proj": round_tensor(torch.empty(hidden_size, num_attention_heads * head_dim).uniform_(-1.1, 1.1)).to(torch.bfloat16),
                "gate": round_tensor(torch.empty(intermediate_size, hidden_size).uniform_(-1.1, 1.1)).to(torch.bfloat16),
                "up": round_tensor(torch.empty(intermediate_size, hidden_size).uniform_(-1.1, 1.1)).to(torch.bfloat16),
                "down": round_tensor(torch.empty(hidden_size, intermediate_size).uniform_(-1.1, 1.1)).to(torch.bfloat16),
            }
        )
    final_norm = round_tensor(torch.empty(hidden_size).uniform_(0.5, 1.5)).to(torch.bfloat16)

    token_ids = torch.tensor([1, 5, 3, 2], dtype=torch.int64)
    hidden_states = embed[token_ids]
    for params in layer_params:
        hidden_states = qwen_decoder_block_module(hidden_states, params["input_ln"], params["post_ln"], params, norm_epsilon)
    hidden_states = rms_norm(hidden_states.float(), final_norm.float().view(1, -1), norm_epsilon).to(torch.bfloat16)
