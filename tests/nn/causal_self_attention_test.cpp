...
TEST_F(CausalSelfAttentionTest, GivenSingleTokenHead_WhenApplyingCausalSelfAttention_ThenValueIsReturned) {
    const Tensor query = make_f32_tensor("query", {1, 1, 2}, {0.5f, -1.0f});
    const Tensor key = make_f32_tensor("key", {1, 1, 2}, {1.0f, 0.25f});
    const Tensor value = make_f32_tensor("value", {1, 1, 2}, {2.0f, -3.0f});
    const Tensor expected = make_f32_tensor("causal_self_attention_result", {1, 1, 2}, {2.0f, -3.0f});

    EXPECT_EQ(expected, causal_self_attention(query.view(), key.view(), value.view()));
}

TEST_F(CausalSelfAttentionTest, GivenTorchOracleF32Inputs_WhenApplyingCausalSelfAttention_ThenExpectedValuesAreReturned) {
    ...
    // PY
    const Tensor query =
        make_f32_tensor("query", {2, 3, 2}, {0.5f, -1.0f, 1.25f, 0.75f, -0.5f, 2.0f, 1.5f, 0.5f, -1.0f, 1.0f, 0.25f,
                                             -0.75f});
    const Tensor key =
        make_f32_tensor("key", {2, 3, 2}, {1.0f, 0.0f, 0.5f, -1.5f, -0.75f, 1.25f, 0.25f, 1.5f, -1.25f, 0.5f, 1.0f,
                                           -0.5f});
    const Tensor value =
        make_f32_tensor("value", {2, 3, 2}, {2.0f, -1.0f, 0.5f, 1.5f, -1.25f, 0.75f, -0.5f, 2.0f, 1.75f, -1.25f, 0.25f,
                                             0.5f});

    const Tensor result = causal_self_attention(query.view(), key.view(), value.view());
...
TEST_F(CausalSelfAttentionTest,
    ...
    // PY
    const Tensor query = make_bf16_tensor("query", {2, 2, 2}, {1.0f, -0.5f, 0.25f, 1.5f, -1.25f, 0.75f, 1.5f, -0.25f});
    const Tensor key = make_bf16_tensor("key", {2, 4, 2},
                                        {0.5f, -1.0f, 1.25f, 0.5f, -0.75f, 1.5f, 1.0f, -0.25f, -0.5f, 1.0f, 1.5f,
                                         -1.25f, 0.75f, 0.25f, -1.0f, 0.5f});
    const Tensor value = make_bf16_tensor("value", {2, 4, 2},
                                          {1.0f, 0.5f, -1.5f, 0.75f, 0.25f, -0.5f, 1.75f, 1.25f, -0.75f, 1.5f, 1.25f,
                                           -0.25f, 0.5f, 0.75f, -1.5f, 1.0f});

    const Tensor result = causal_self_attention(query.view(), key.view(), value.view(), 2);

    EXPECT_EQ(std::string("causal_self_attention_result"), result.tensor_info().name);
    EXPECT_EQ(DType::BF16, result.tensor_info().dtype);
    EXPECT_EQ(Shape({2, 2, 2}), result.tensor_info().shape);
    expect_float_values_near(result.view(),
                             {-0.2109375f, 0.53515625f, -0.0161132812f, 0.1000976562f, -0.451171875f, 1.296875f,
                              0.87109375f, 0.123046875f},
                             1e-6f);
}

TEST_F(CausalSelfAttentionTest, GivenMismatchedPastLength_WhenApplyingCausalSelfAttention_ThenItThrows) {
    const Tensor query = make_f32_tensor("query", {1, 2, 2}, {1.0f, 0.5f, -0.5f, 1.5f});
    const Tensor key = make_f32_tensor("key", {1, 2, 2}, {0.25f, 1.0f, -1.25f, 0.5f});
    const Tensor value = make_f32_tensor("value", {1, 2, 2}, {1.5f, -0.25f, 0.5f, 0.75f});
